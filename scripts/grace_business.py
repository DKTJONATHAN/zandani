#!/usr/bin/env python3
"""Grace Wambui — straight business reporter. No commentary."""
...
STYLE_PRESETS = [
    ...
    {"name": "Market Report", "lead_style": "Open with the number, deal or policy move.",
...
BRANDS_TO_SCRUB = [
    ...
    "BBC", "CNN", "Reuters", "Al Jazeera", "Business Daily",
]
    ...
    try:
        ...
        for a in soup.select("a[href]")[:80]:
            ...
            if len(title) < 25 or len(title) > 140:
                continue
            if not any(x in href for x in ["/20", "article", "news", "story", "post", "bd"]):
                continue
            if href.startswith("/"):
