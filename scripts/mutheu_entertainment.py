...
def scrub_brands(text):
    for b in ["Pulse Live", "Nation.Africa", "Daily Nation", "The Standard", "Tuko", "Kenyans.co.ke"]:
...
def scrape_source():
    ...
    try:
        ...
        for a in soup.select("a[href]")[:80]:
            ...
            if SOURCE_DOMAIN not in href and "pulselive" not in href:
                continue
...
def scrape_article(url):
    try:
        ...
        text = ""
        for sel in ["article", ".article-body", ".story-body", "main article", ".content"]:
...
def main():
    ...
    for story in stories[:MAX_SCRAPE_TRIES]:
        blob = story.get("title", "")
        if should_skip_story(blob, CATEGORY):
            print(f"Skip (not Kenya-first): {blob[:80]}")
            continue
        text, img, ttl = scrape_article(story["url"])
        if not text or len(text) < 200:
            continue
        if should_skip_story(blob + " " + text, CATEGORY):
            continue
        try:
            prompt = news_prompt(AUTHOR_NAME, full_date_str, style, ttl or blob, text, role="correspondent", desk=CATEGORY)
        ...
        if h in memory.get("published_hashes", []):
            ...
            continue
        title = ttl or story["title"]
        if article.startswith("#"):
            first = article.split("\n", 1)[0]
            title = re.sub(r"^#+\s*", "", first).strip() or title
            article = article.split("\n", 1)[-1].strip()
        write_post(title, article, style["name"], story["url"], img or "")
