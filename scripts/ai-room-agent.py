...
def history(chat):
    return "\n\n".join(
        f"[{m['speaker'].upper()} — round {m.get('round', 0)}]\n{m['text']}"
...
def call_gemini(chat):
    ...
    payload = {
        "contents": [{"role": "user", "parts": [{"text": prompt_for("gemini", chat)}]}],
...
def call_openai(chat):
    ...
    payload = {
        "model": model,
        "input": [{"role": "user", "content": prompt_for("chatgpt", chat)}],
    }
    request = urllib.request.Request(url, data=json.dumps(payload).encode(), headers={
...
def main():
    ...
    expected = "awaiting_gemini" if provider == "gemini" else "awaiting_chatgpt"
    if chat.get("status") != expected:
        print(f"skip: status is {chat.get('status')}, expected {expected}")
        return
    ...
    try:
        ...
        chat["messages"].append({
            "id": f"{provider}-{chat['round']}-{int(datetime.now().timestamp())}",
            "speaker": provider,
            "text": reply,
            "created_at": now(),
            "round": chat["round"],
